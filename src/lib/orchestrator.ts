...
export async function runDiagnostic(input: {
  ...
  // 1. Resolve a point. A folio-only search goes straight to the FL DOR roll by
  //    parcel id - geocoding a bare folio string just lands on the state
  //    centroid (Polk County), which used to make the folio field look broken.
  //    Address searches geocode as usual.
  const folioInput = (input.folio ?? '').trim();
  let geo: GeocodeResult;
  if (!rawAddress && folioInput) {
    const fr = await parcelByFolio(folioInput);
    if (!fr) {
      throw new Error(
        `Could not locate folio "${folioInput}" in the state parcel roll. Check the number, or search by street address instead.`,
      );
    }
    geo = {
      lat: fr.lat,
      lng: fr.lng,
      formattedAddress: fr.parcel.siteAddress ?? `Folio ${folioInput}`,
      county: fr.parcel.countyKey,
      countyDisplay: null,
      source: 'census',
      partialMatch: false,
      locationType: null,
    ...
    const g = await geocode(rawAddress);
    if (!g) {
      throw new Error(
  ...
  // If no county came back (Census fallback), reverse-lookup via FCC.
  ...
  {
    // Build the property summary only from the fields we actually have, so a
    // parcel with missing roll data never renders as "?-built, ? heated sq ft".
    const desc = adapterResult.propertyBasics.dorDescription ?? 'property';
    const head = yearBuilt ? `${yearBuilt}-built ${desc}` : desc;
    let sizeStr = '';
    if (heated && lot) sizeStr = `${heated.toLocaleString()} heated sq ft on a ${lot.toLocaleString()} sq ft lot`;
    else if (heated) sizeStr = `${heated.toLocaleString()} heated sq ft`;
    else if (lot) sizeStr = `${lot.toLocaleString()} sq ft lot`;
    if (yearBuilt || heated || lot) {
      bottomLine.push(sizeStr ? `${head}, ${sizeStr}.` : `${head}.`);
    } else {
      bottomLine.push(
        `Property details (year built, size, lot) were not returned for this parcel from the state certified roll - confirm with the county Property Appraiser.`,
      );
    }
  }
